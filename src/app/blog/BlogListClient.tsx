...
const CONTINENTS: Record<string, { label: string; emoji: string; countries: string[] }> = {
  asia: {
    label: 'Азия',
    emoji: '🌏',
    countries: ['Япония', 'Тайланд', 'Китай', 'Сингапур', 'Индия', 'Виетнам', 'Малайзия', 'ОАЕ', 'Индонезия', 'Камбоджа', 'Непал', 'Шри Ланка', 'Мианмар', 'Лаос', 'Бахрейн', 'Катар', 'Саудитска Арабия', 'Йордания', 'Израел', 'Монголия', 'Казахстан', 'Узбекистан', 'Таджикистан', 'Киргизстан', 'Туркменистан', 'Азербайджан', 'Армения', 'Грузия', 'Иран', 'Ирак', 'Сирия', 'Ливан', 'Кипър', 'Бруней', 'Тайван', 'Южна Корея', 'Северна Корея', 'Бутан', 'Бангладеш', 'Пакистан', 'Афганистан', 'Малдиви', 'Филипини', 'Источен Тимор'],
  },
  europe: {
    label: 'Европа',
    emoji: '🌍',
    countries: ['Гърция', 'Италия', 'Испания', 'Франция', 'Германия', 'Хърватия', 'Австрия', 'Швейцария', 'Швеция', 'Норвегия', 'Финландия', 'Дания', 'Нидерландия', 'Белгия', 'Португалия', 'Полша', 'Чехия', 'Унгария', 'Румъния', 'България', 'Словакия', 'Словения', 'Хърватия', 'Босна и Херцеговина', 'Сърбия', 'Черна гора', 'Косово', 'Северна Македония', 'Албания', 'Молдова', 'Украйна', 'Беларус', 'Русия', 'Исландия', 'Ирландия', 'Великобритания', 'Малта', 'Люксембург', 'Монако', 'Лихтенщайн', 'Андора', 'Ватикан', 'Сан Марино', 'Естония', 'Латвия', 'Литва', 'Турция'],
  },
  america: {
    label: 'Америка',
    emoji: '🌎',
    countries: ['САЩ', 'Мексико', 'Перу', 'Бразилия', 'Аржентина', 'Куба', 'Колумбия', 'Чили', 'Еквадор', 'Боливия', 'Парагвай', 'Уругвай', 'Венецуела', 'Гватемала', 'Коста Рика', 'Панама', 'Никарагуа', 'Хондурас', 'Ел Салвадор', 'Хаити', 'Доминиканска република', 'Ямайка', 'Тринидад и Тобаго', 'Барбадос', 'Канада', 'Бelize', 'Гвиана', 'Суринам'],
  },
  africa: {
    label: 'Африка',
    emoji: '🌍',
    countries: ['Южна Африка', 'ЮАР', 'Мароко', 'Египет', 'Кения', 'Танзания', 'Уганда', 'Руанда', 'Намибия', 'Ботсвана', 'Зимбабве', 'Замбия', 'Мадагаскар', 'Мозамбик', 'Гана', 'Нигерия', 'Сенегал', 'Тунис', 'Алжир', 'Либия', 'Судан', 'Еритрея', 'Сомалия', 'Джибути', 'Етиопия', 'Мали', 'Буркина Фасо', 'Нигер', 'Чад', 'Камерун', 'Централноафриканска република', 'Ангола', 'Конго', 'Кения', 'Сейшели', 'Мавриций', 'Кабо Верде', 'Сао Томе и Принсипи', 'Мавритания', 'Гамбия', 'Либерия', 'Кот д\'Ивоар', 'Гвинея', 'Гвинея-Бисау', 'Бенин', 'Того', 'Лесото', 'Есватини', 'Малави', 'Южен Судан'],
  },
  oceania: {
    label: 'Океания',
    emoji: '🌊',
...
function PostCard({ post, featured = false }: { post: BlogPost; featured?: boolean }) {
    ...
    return (
      <Link href={href} className="group block rounded-[2.5rem] overflow-hidden shadow-xl border border-brand-gold/10 hover:shadow-2xl transition-all duration-500">
        <div className="grid grid-cols-1 lg:grid-cols-2 min-h-[380px]">
          {/* Image */}
          <div className="relative h-[280px] lg:h-full overflow-hidden bg-brand-dark">
            {post.coverImg && (
              <Image
                src={post.coverImg}
                alt={post.title}
            ...
            <div className="absolute top-5 left-5 flex items-center gap-2">
              <span className="bg-brand-gold text-brand-dark px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest shadow">
                Последна публикация
              </span>
              {post.relatedCountry && (
                <span className="bg-white/90 backdrop-blur-sm text-brand-dark px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest shadow">
                  {post.relatedCountry}
                </span>
              )}
            </div>
          ...
          <div className="bg-brand-dark text-white p-10 lg:p-14 flex flex-col justify-center gap-4">
            ...
            <h2 className="text-3xl lg:text-4xl font-serif italic text-white leading-tight group-hover:text-brand-gold transition-colors duration-300">
              {post.title}
            </h2>
  ...
  return (
    <Link href={href} className="group block bg-white rounded-[1.8rem] overflow-hidden shadow-sm hover:shadow-xl border border-brand-gold/8 hover:border-brand-gold/25 transition-all duration-400 hover:-translate-y-0.5">
      <div className="relative h-52 overflow-hidden bg-brand-dark/10">
        ...
        {post.relatedCountry && (
          <div className="absolute bottom-3 left-3 bg-white/90 backdrop-blur-sm text-brand-dark px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-wider shadow-sm flex items-center gap-1">
            <MapPin size={8} className="text-brand-gold" />
            {post.relatedCountry}
          </div>
        )}
      </div>
      ...
      <div className="p-6">
        <div className="flex items-center gap-2 text-[9px] font-bold uppercase text-gray-400 tracking-wider mb-3">
          <Clock size={10} className="text-brand-gold/70" />
          <span>{date}</span>
          {post.readTime && <><span className="text-gray-300">·</span><span>{post.readTime} мин.</span></>}
        </div>

        <h3 className="font-serif italic text-xl text-brand-dark mb-3 leading-snug group-hover:text-brand-gold transition-colors line-clamp-2">
          {post.title}
        </h3>
...
function DestinationTile({ country, posts, img, onClick }: { country: string; posts: BlogPost[]; img?: string; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="group relative rounded-2xl overflow-hidden bg-brand-dark shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-0.5 aspect-[4/3] w-full text-left"
    >
      {img && (
        <Image src={img} alt={country} fill className="object-cover opacity-60 group-hover:opacity-75 group-hover:scale-105 transition-all duration-500" sizes="200px" />
      )}
      {!img && (
...
export default function BlogListClient({ posts }: { posts: BlogPost[] }) {
  ...
  // Extract countries that actually appear in posts
  ...
  // Countries for the active continent filter
  ...
  // Grouped by continent for "destination" mode
  const grouped = useMemo(() => {
    ...
    Object.entries(CONTINENTS).forEach(([key, cont]) => {
      ...
      cont.countries.forEach(c => {
        if (postCountries[c]) countries[c] = postCountries[c];
      });
      if (Object.keys(countries).length > 0) {
        result[key] = { continent: cont, countries };
      }
    });
    return result;
  }, [postCountries]);
  ...
  return (
    <div className="min-h-screen bg-[#f9f6f0]">
      ...
      {/* ── Page header ── */}
      <div className="bg-brand-dark pt-32 pb-12 px-6">
        <div className="container mx-auto">
          <div className="flex items-center gap-3 mb-4">
            <div className="h-px w-8 bg-brand-gold/40" />
            <span className="text-brand-gold/60 text-[9px] font-black uppercase tracking-[0.4em]">Beliva VIP Tour</span>
          </div>
          ...
          <div className="container mx-auto px-6 pb-3 overflow-x-auto">
            <div className="flex items-center gap-2 min-w-max">
              ...
              <button
                ...
                className={`text-[10px] font-black uppercase tracking-wider px-4 py-2 rounded-full border transition-all whitespace-nowrap ${
                  !hasFilters
                    ? 'bg-brand-dark text-brand-gold border-brand-dark'
                    : 'border-brand-gold/30 text-brand-dark/60 hover:border-brand-gold hover:text-brand-dark'
                }`}
              >
                ✦ Всички
              </button>
              ...
              {/* Continent pills */}
              {Object.entries(CONTINENTS).filter(([key]) => grouped[key]).map(([key, cont]) => (
                <button
                  key={key}
                  ...
                  className={`text-[10px] font-black uppercase tracking-wider px-4 py-2 rounded-full border transition-all whitespace-nowrap ${
                    selectedContinent === key
                      ? 'bg-brand-dark text-brand-gold border-brand-dark'
                      : 'border-gray-200 text-gray-500 bg-gray-50 hover:border-brand-gold/40 hover:text-brand-dark'
                  }`}
                >
              ...
              {/* Country pills — only when continent is selected */}
                    ...
                    <button
                      ...
                      className={`text-[10px] font-black uppercase tracking-wider px-4 py-2 rounded-full border transition-all whitespace-nowrap ${
                        selectedCountry === c
                          ...
                          : 'border-brand-gold/20 text-brand-dark/70 bg-brand-gold/5 hover:bg-brand-gold/15'
                      }`}
                    >
                      {c}
                      <span className="ml-1.5 text-[8px] opacity-60">
                        {postCountries[c]?.length}
                      </span>
                    </button>
                  ))}
                </>
            ...
            >
              {filteredPosts.length === 0 && (
                <div className="text-center py-24">
                  <p className="text-gray-400 text-lg font-serif italic mb-3">Все още няма статии за тази дестинация.</p>
              ...
              {/* Featured */}
              {featuredPost && !selectedCountry && !selectedContinent && (
                <div className="mb-12">
              ...
              {gridPosts.length > 0 && (
                <>
                  {(selectedContinent || selectedCountry) && filteredPosts.length > 0 && (
                    <div className="mb-8">
                      <PostCard post={filteredPosts[0]} featured />
                    </div>
                  )}
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7">
                    {((selectedContinent || selectedCountry) ? filteredPosts.slice(1) : gridPosts).map(post => (
                      <motion.div
                        key={post.id}
              ...
              {/* Only 1 post and no filter */}
              {filteredPosts.length === 1 && !selectedContinent && !selectedCountry && (
                <PostCard post={filteredPosts[0]} featured />
              )}
            </motion.div>
          </AnimatePresence>
